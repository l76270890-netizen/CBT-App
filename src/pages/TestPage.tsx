...
import { questionsBank } from '../data/questionsBank' // <-- ADD THIS FILE
...
export default function Test({ setActivePage, testConfig }: Props) {
  ...
  const singleSubject = (testConfig as any).subject || subjectsList[0]
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      ...
      if (fetched.length === 0) {
        try {
          const subjectsParam = singleSubject || subjectsList.join(',')
          const res = await fetch(`${API_URL}/api/questions?examType=${examType}&subjects=${subjectsParam}&topic=${filterTopic || ''}`)
          if (res.ok) {
        ...
        } catch (e) { console.log("API failed, using local bank") }
      }

      // 3. Fallback to Local Real Bank
      if (fetched.length === 0) {
        const bankKey = singleSubject?.toLowerCase() || 'english'
        let bankQs = questionsBank[bankKey] || questionsBank['english'] || []

        // Topic filter
        if (filterTopic) {
          const filtered = bankQs.filter(q => q.topic?.toLowerCase() === filterTopic.toLowerCase())
      ...
      if (fetched.length === 0) {
        fetched = Array.from({ length: testConfig.totalQuestions || 10 }).map((_, i) => ({
          id: `mock-${i}`, subject: singleSubject, question: `${singleSubject}: Question ${i + 1} - Add real questions in data/questionsBank.ts`, options: ['Option A','Option B','Option C','Option D'], answer: i % 4, explanation: 'Update questionsBank.ts'
        }))
      }
  ...
  if (loading) return <div className="test-page1" style={{color:'#fff',padding:40,textAlign:'center'}}>Loading {filterTopic? filterTopic : singleSubject} questions...<br/>Please wait</div>
  ...
  return (
    <div className="test-page1">
      ...
      {showGrid && <div className="grid-overlay" onClick={()=>setShowGrid(false)}><div className="grid-modal" onClick={e=>e.stopPropagation()}><h3>Questions - {singleSubject} {filterTopic? `(${filterTopic})` : ''}</h3><div className="question-grid">{questions.map((_,i)=><button key={i} className={`grid-item ${answers[i]!==null?'answered':''} ${current===i?'active':''}`} onClick={()=>{setCurrent(i); setShowGrid(false)}}>{i+1}</button>)}</div><button className="submit-btn full" onClick={handleSubmit}>Submit Test</button></div></div>}
